...
app.get("/api/v1/topics", asyncHandler(async (_req, res) => {
  const query = `
...
app.get("/api/v1/topics/:topicId/questions", asyncHandler(async (req, res) => {
  const { topicId } = req.params;
  const requestedLimit = req.query.limit ? Number(req.query.limit) : defaultQuizQuestionLimit;
  ...
  const questionsResult = await pool.query(questionsQuery, [topicId, requestedLimit]);
...
app.get("/api/v1/highscores", asyncHandler(async (req, res) => {
...
app.post("/api/v1/highscores", asyncHandler(async (req, res) => {
  const { topicId, playerName, score, totalQuestions, percentage, durationSeconds } = req.body;
  ...
  if (!playerName || typeof playerName !== "string" || playerName.length > 40) {
    res.status(400).json({ error: "playerName is required and must be <= 40 characters" });
    return;
  }
  ...
  if (!Number.isInteger(percentage) || percentage < 0 || percentage > 100) {
    res.status(400).json({ error: "percentage must be an integer between 0 and 100" });
    return;
  }

  if (!Number.isInteger(durationSeconds) || durationSeconds < 0) {
    res.status(400).json({ error: "durationSeconds must be an integer >= 0" });
    return;
  }

  const topicExists = await pool.query("SELECT 1 FROM topics WHERE id = $1", [topicId]);
...
app.post("/api/v1/import/questions", asyncHandler(async (req, res) => {
  ...
  if (!Array.isArray(questions) || questions.length < 1) {
    res.status(400).json({ error: "questions array with at least one item is required" });
    return;
  }
  ...
  if (!isValidTopicSlug(topicId)) {
    res.status(400).json({ error: "topic.id must be a slug ([a-z0-9-], max 50)" });
    return;
  }

  if (!isNonEmptyString(topicName, 100)) {
    res.status(400).json({ error: "topic.name is required and must be <= 100 characters" });
    return;
  }

  if (!isNonEmptyString(topicDescription, 300)) {
    res.status(400).json({ error: "topic.description is required and must be <= 300 characters" });
    return;
  }

  if (!isNonEmptyString(topicIcon, 8)) {
    res.status(400).json({ error: "topic.icon is required and must be <= 8 characters" });
    return;
  }
  ...
  for (let i = 0; i < questions.length; i += 1) {
    ...
    if (!isNonEmptyString(item.question, 300)) {
      res.status(400).json({ error: `questions[${i}].question is required and must be <= 300 characters` });
      return;
    }

    if (!Array.isArray(item.options) || item.options.length < 2 || item.options.length > 6) {
      res.status(400).json({ error: `questions[${i}].options must contain 2 to 6 items` });
      return;
    }

    const optionsAreValid = item.options.every((option) => isNonEmptyString(option, 120));
    if (!optionsAreValid) {
      res.status(400).json({ error: `questions[${i}].options contains invalid values` });
      return;
    }

    if (!Number.isInteger(item.correctIndex) || item.correctIndex < 0 || item.correctIndex >= item.options.length) {
      res.status(400).json({ error: `questions[${i}].correctIndex must be an in-range integer` });
      return;
    }
  }
  ...
  try {
    ...
    const existingTopic = await client.query("SELECT 1 FROM topics WHERE id = $1", [topicId]);
    ...
    for (const item of questions) {
      ...
      const deleteExisting = await client.query(
        `
          ...
          RETURNING id
        `,
        [topicId, normalizedQuestion, JSON.stringify(normalizedOptions), item.correctIndex]
      );
